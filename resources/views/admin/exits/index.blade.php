...
@section('content')
    <div class="flex space-x-2 items-center">
        <span class="bi bi-fullscreen-exit text-2xl"></span>
        <h2 class="text-xl">Exit Countries</h2>
    </div>
    <div class="mt-4 bg-white">
        <div class="bg-blue-500  p-2 flex justify-between">
            <h2 class="text-white">Exit Countries</h2>
            <a class="p-2 bg-white rounded-md text-xs" href="#">Add Exit Country</a>
        </div>
        <div class="px-4 pb-2">
                        ...
                        @foreach ($collection as $item)
                            <tr class="border-b-2">
                                ...
                                <td class="flex space-x-1">
                                    <form action="{{ route('exits.update', $item->id) }}" method="post">
                                        @method('put')
                                        @csrf
                                        <input type="hidden" name="status" value="accept">
                                        <button type="submit"><span class="bi bi-check"></span></button>
                                    </form>
                                    <form action="{{ route('exits.update', $item->id) }}" class="rejectForm" method="post">
                                        @method('put')
                                        @csrf
