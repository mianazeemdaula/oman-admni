...
@section('content')
    <div class="grid grid-cols-5 p-4 gap-4">
        ...
        <x-stat-card title="إجمالي التراث المعماري" count="{{ \App\Models\Building::count() }}" color="bg-green-500">
            <span class="bi bi-building text-white"></span>
        </x-stat-card>
        <x-stat-card title="إجمالي المقتنيات" count="{{ \App\Models\Item::count() }}" color="bg-red-500">
            <span class="bi bi-rocket text-white"></span>
        </x-stat-card>
        <x-stat-card title="إجمالي المؤسسات" count="{{ \App\Models\Organization::count() }}" color="bg-yellow-500">
            <span class="bi bi-shop text-white"></span>
        </x-stat-card>
        <x-stat-card title="إجمالي الأفراد" count="{{ \App\Models\User::count() }}" color="bg-purple-500">
