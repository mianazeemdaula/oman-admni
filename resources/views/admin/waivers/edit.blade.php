    ...
    <form action="{{ route('admins.update', $admin->id) }}" method="post" enctype="multipart/form-data">
        ...
        <div class="mt-4 bg-white">
            <div class="bg-blue-500  p-2">
                <h2 class="text-white">Edit Admin</h2>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 p-2 gap-2 md:p-4">
                <div>
                    <h3 class="p-1">Username</h3>
                    <input type="text" placeholder="Username" name="username" value="{{ $admin->username }}"
                        class="w-80">
                    @error('username')
                ...
                <div>
                    ...
                    <input type="password" placeholder="Password" name="password" class="w-80">
                    @error('password')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
